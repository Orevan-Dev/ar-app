...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
    ...
    private bool ReInitializeFlag = false;
    ...
    {
        //We need to re initialize in late update to give some time for Gameobject.Destroy in case of changes in child count
    ...
    void LateUpdate()
    {
        if(ReInitializeFlag)
        {
            if (_container == null)
            {
                Debug.LogError("Container can not be null");
                return;
            }
    ...
    //------------------------------------------------------------------------
    private void SetPagePositions(bool firstTime) {
        ...
        if (_horizontal) {
            ...
            //Debug.Log("Page Count: " + _pageCount + " Width: " + width  + "Screen Width: " + Screen.width);
            // limit fast swipe length - beyond this length it is fast swipe no more
            _fastSwipeThresholdMaxLimit = (int)width;
        } else {
            containerHeight = height * _pageCount;
            _fastSwipeThresholdMaxLimit = (int)height;
        }

        Vector2 newSize = new Vector2(containerWidth, containerHeight);
        _container.sizeDelta = newSize;

        /** Changing anchor position from center to scale made us in no need to set the positions **/
        ...
        for (int i = 0; i < _pageCount; i++) {
            ...
            if (_horizontal) {
                childPosition = new Vector2((i * width) - (_pageCount/2 * width) + width/2, 0f);
            } else {
                childPosition = new Vector2(0f, -(i * height - containerHeight / 2));
            }
            child.anchoredPosition = childPosition;
            //Set Child Size = Scroll Rect Initial size to scale with screen
            //I noticed that the child size needs different calculation if it was created in runtime
            //I do not know why but this is a work around till I figure it out
            if(scale)
                child.sizeDelta = new Vector2(width * scaleFactor, height * scaleFactor);
            else
    ...
    private void SetPageSelection(int aPageIndex) {
        ...
        if (skipText != null)
        {
            UpdateSkipText(aPageIndex);
        }
    }

    private void UpdateSkipText(int aPageIndex)
    {
        //if (LanguageParse.langinstance.CurrentLanguage == "en")
        {
            if (aPageIndex == _pageCount - 1)
            {
                skipText.text = "OK";
                skipText.color = new Color(237 / 255f, 105 / 255f, 105 / 255f);

            }
            else
            {
                skipText.text = "SKIP";
                skipText.color = new Color(248 / 255f, 204 / 255f, 204 / 255f);
            }
        }
       /* else
        {
